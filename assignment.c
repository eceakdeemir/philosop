...
int variable_assignment(t_main_struct *main_struct, char **av)
{
    ...
    if (!av[5])
        main_struct->number_of_times_to_eat = -1;
    else
        main_struct->number_of_times_to_eat = ft_atoi(av[5]);
	main_struct->all_philos = malloc(sizeof(t_philo) * main_struct->number_of_philo);
	if (!main_struct->all_philos)
        return (-1);
    main_struct->forks_mutex = malloc(sizeof(pthread_mutex_t) * main_struct->number_of_philo);
